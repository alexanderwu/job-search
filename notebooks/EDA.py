...
from job_search.config import P_CACHE, P_DICT, P_QUERY, _date
...
_location = (
    job_v5.str["formatted_workplace_location"]
...
aw.displays(
    aw.combo_sizes(
        [bay_remote_set, yoe_45d_set, _DS_set, health_set],
        ["Bay_Remote", "yoe_45d", "DS", "Health"],
    ),
    aw.combo_sizes2(
        [bay_remote_set, yoe_45d_set, _DS_set, health_set],
        ["Bay_Remote", "yoe_45d", "DS", "Health"],
...
_DS_df = (
    job_df[_DS_mask]
    .sort_values(["_est_pub_date", "_ctime"], ascending=False)
    .drop_duplicates("_hash")
)
DS_df = (
    job_df[DS_mask]
    .sort_values(["_est_pub_date", "_ctime"], ascending=False)
...
# %%
DL_mask = job_df["technical_tools"].apply(
    lambda x: bool(
        set(x) & {"PyTorch", "TensorFlow", "LangChain", "LLMs", "JAX", "Keras", "Hugging Face"}
    )
    if x
    else False
)
...
def viewhash(hash):
    row_df = merged_df.query("_hash == @hash").sort_values(
        ["_est_pub_date", "_ctime"], ascending=False
    )
    row = row_df.iloc[0]
